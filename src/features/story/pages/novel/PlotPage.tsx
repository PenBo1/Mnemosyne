...
export function PlotPage() {
  ...
  const getDotVariant = (status: string): "default" | "primary" | "secondary" | "destructive" | "muted" => {
    switch (status) {
  ...
  return (
    <PageContainer scrollable={false}>
      ...
      <Tabs value={view} onValueChange={(v) => setView(v as "outline" | "timeline" | "tree")} className="flex-1">
        <TabsList>
                  ...
                  <CardContent className="flex items-center gap-3 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-[10px] uppercase">{t.plot.types[p.type]}</Badge>
        ...
        <TabsContent value="timeline" className="flex-1">
          {loading ? (
            <LoadingState label={t.common.loading} />
          ) : sortedByChapter.length === 0 ? (
            <EmptyState icon={<ClockIcon />} title={t.plot.empty} />
          ) : (
            <Timeline>
              {sortedByChapter.map((p, idx) => (
                <TimelineItem key={p.id} onClick={() => openEdit(p)} className="cursor-pointer group">
                  <TimelineSeparator>
                    <TimelineDot variant={getDotVariant(p.status)} />
                    {idx < sortedByChapter.length - 1 && <TimelineConnector />}
                  </TimelineSeparator>
                  <TimelineContent>
                    <TimelineHeader>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant="outline" className="text-xs">{t.plot.types[p.type]}</Badge>
                  ...
                  <Card key={type}>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm flex items-center gap-2">
                        <Badge variant="outline">{t.plot.types[type as PlotPointType]}</Badge>
                    ...
                    <CardContent className="flex flex-col gap-1">
                      {items.map((p) => (
                        <div
                          key={p.id}
                          onClick={() => openEdit(p)}
                          className="flex items-center gap-2 cursor-pointer hover:bg-[var(--bg-overlay-l2)] rounded px-2 py-1 group"
                        >
