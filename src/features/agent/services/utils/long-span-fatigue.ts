...
//
// 迁移要点：
// - import 路径：`./chapter-cadence.js` → `./chapter-cadence`；`./cadence-policy.js` → `./cadence-policy`（同目录去 .js 后缀）。
// - I/O 改造（node:fs/promises + node:path → Tauri IPC + joinPath）：
//   - readFile(path, "utf-8") → ipc<string>("fs_read_file", { path }).catch(() => "")（失败返回空串）
//   - readdir(path) → ipc<FileEntry[]>("fs_list_directory", { path }).catch(() => []) 后取 entries.map(e => e.name)
//   - join(...) → joinPath(...)
//   - 原 try/catch 返回空的错误处理由 .catch 等价替代（任务约定：.catch(() => "") 与原 try/catch 返回空语义等价）。
// - 业务逻辑零改动（正则、阈值常量、dice 系数、English stop words、issue 构建逻辑全部保留）。
//
// 注意：硬编码中英文 prompt 字符串是 LLM 诊断告警文本（非 UI 字符串），
// 与 architect-prompts.ts 一致，保留原样不强制 i18n。

import { ipc } from "@/services/ipc";
import type { FileEntry } from "@/types/app";
import { joinPath } from "./path-utils";
import { analyzeChapterCadence } from "./chapter-cadence";
import {
...
export async function buildEnglishVarianceBrief(params: {
  ...
  const summaryRows = await loadSummaryRows(joinPath(params.bookDir, "story", "chapter_summaries.md"));
...
export async function analyzeLongSpanFatigue(
  ...
  const summaryRows = await loadSummaryRows(joinPath(input.bookDir, "story", "chapter_summaries.md"));
...
async function loadPreviousChapterBodies(
  ...
  const entries = await ipc<FileEntry[]>("fs_list_directory", { path: chaptersDir }).catch(() => []);
  ...
  return Promise.all(
    previousFiles.map((entry) =>
      ipc<string>("fs_read_file", { path: joinPath(chaptersDir, entry.file) }).catch(() => ""),
    ),
  );
}
