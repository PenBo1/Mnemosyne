// Writer/Auditor 提示词真相文件智能过滤。
//
// 迁移要点（import 路径调整）：
// - `./chapter-cadence.js`（常量 DEFAULT_CHAPTER_CADENCE_WINDOW）→ `./chapter-cadence`
//   （chapter-cadence.ts 已迁移到同目录）
// - 业务逻辑零改动（capContextBlock + filterHooks + filterSummaries + filterSubplots +
//   filterEmotionalArcs + filterCharacterMatrix + 内部 helper 全部保留）。
...
import { DEFAULT_CHAPTER_CADENCE_WINDOW } from "./chapter-cadence";
...
export function capContextBlock(content: string, options: ContextCapOptions): string {
  ...
  const note = `\n\n[context budget: omitted about ${omitted} chars from ${options.label}; kept beginning and latest tail.]\n\n`;
