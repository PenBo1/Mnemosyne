// Spot-Fix Patches —— 纯函数 + 类型。
//
// 修订器 spot-fix 补丁解析与应用：
// - parseSpotFixPatches: 从 === PATCHES === 段提取 TARGET_TEXT/REPLACEMENT_TEXT 配对
// - applySpotFixPatches: best-effort 逐补丁应用（先精确匹配，再空白归一化模糊匹配，
//   单个补丁失败时跳过而不拒绝整批）
//
// 完全独立无 import。业务逻辑零改动（正则、字符位置映射算法均保持原样）。
//
// 迁移要点：删除原源文件中两处死代码：
//   1. `tryFuzzyMatch` 内未使用的 `const contentChars = [...content];`
//   2. `mapNormalizedToOriginal` 内声明但从未读取的 `let leadingSkipped = false;`
//   Mnemosyne 启用 noUnusedLocals 会报错；删除不影响任何运行时行为。
