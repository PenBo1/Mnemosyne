// Hook stale/blocked 检测。
//
// 迁移要点（import 路径调整）：
// - `../state/memory-db.js`（类型 StoredHook）→ `@/shared/types/hook`
// - `../models/runtime-state.js`（类型 HookRecord）→ `@/shared/types/runtime-state`
// - `./hook-promotion.js`（resolveHalfLifeChapters）→ `./hook-promotion`（已迁移）
//
// 纯函数（computeHookDiagnostics + renderHookDiagnosticMarker），无 I/O，无外部依赖，
// 业务逻辑零改动。
