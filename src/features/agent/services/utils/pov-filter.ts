// POV Filter —— 纯函数。
//
// POV 视角过滤：基于当前 POV 角色的"信息边界"过滤 truth 文件内容。
//   - extractPOVFromOutline: 从卷纲中按章节号提取 POV 声明
//   - filterMatrixByPOV: 信息边界表只保留 POV 角色行，其他角色隐藏
//   - filterHooksByPOV: 隐藏 POV 角色不在场的章节埋下的伏笔
//
// 完全独立无 import。业务逻辑零改动（正则、表格行过滤逻辑均保持原样）。
