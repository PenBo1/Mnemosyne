...
//
// 迁移要点：
// - import 路径：`./cadence-policy.js` → `./cadence-policy`（同目录去 .js 后缀）。
// - 业务逻辑零改动（正则、阈值常量、token 提取逻辑全部保留）。
// - 兼容适配：analyzeScenePressure 中 `types.at(-1)`（ES2022 Array.prototype.at）
//   改为 `types[types.length - 1]`，因 Mnemosyne tsconfig lib 为 ES2020。
//   逻辑等价（取最后一个元素），非业务逻辑改动。
...
function analyzeScenePressure(
  ...
  // 兼容适配：原代码使用 types.at(-1)（ES2022 API），Mnemosyne tsconfig lib 为 ES2020，
  // 改为等价的 length-1 索引访问。逻辑完全相同（取最后一个元素，此处已有 length<2 守卫）。
  const repeatedType = types[types.length - 1];
  if (!repeatedType) {
    return undefined;
  }
