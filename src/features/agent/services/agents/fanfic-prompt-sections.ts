// 同人写作 prompt 片段工厂。
//
// 业务逻辑零改动（buildFanficCanonSection / buildCharacterVoiceProfiles /
// buildFanficModeInstructions + MODE_PREAMBLES / MODE_CHECKS 常量全部保留）。
//
// 注意：硬编码中文 prompt 字符串是 LLM prompt 模板（非 UI 字符串），
// 与 architect-prompts.ts 一致，保留原样不强制 i18n。

import type { FanficMode } from "@/types/book";
