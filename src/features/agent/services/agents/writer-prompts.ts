// Writer Agent System Prompt 组装（大文件，prompt 组装聚合）。
//
// 大文件处理：保持原样，不拆分。文件内部包含十余个 prompt 段落构建函数
// （核心规则/章节备忘对齐/字数治理/写作铁律/创作宪法/代入感六支柱/黄金三章五章/中英文输出格式等）。
//
// 业务逻辑零改动（prompt 文本、正则、表格结构全部保留）。
//
// 注意：硬编码中英文 prompt 字符串是 LLM prompt 模板（非 UI 字符串），
// 与 architect-prompts.ts 一致，保留原样不强制 i18n。

import type { BookConfig, FanficMode } from "@/types/book";
import type { GenreProfile } from "@/types/genre-profile";
import type { BookRules } from "@/types/book-rules";
import type { LengthSpec } from "@/types/length-governance";
import { buildFanficCanonSection, buildCharacterVoiceProfiles, buildFanficModeInstructions } from "./fanfic-prompt-sections";
import { buildEnglishCoreRules, buildEnglishGenreIntro } from "./en-prompt-sections";
import { buildLengthSpec } from "../utils/length-metrics";
...
export function buildAntiAIExamples(): string {
  return `## 去AI味：反例→正例对照
...
export function buildCharacterPsychologyMethod(): string {
  return `## 六步走人物心理分析
...
export function buildSupportingCharacterMethod(): string {
  return `## 配角设计方法论
...
export function buildReaderPsychologyMethod(): string {
  return `## 读者心理学框架
...
export function buildEmotionalPacingMethod(): string {
  return `## 情感节点设计
...
export function buildImmersionTechniques(): string {
  return `## 代入感技法
...
export function buildPreWriteChecklist(_book: BookConfig, gp: GenreProfile): string {
  let idx = 1;
  const lines = [
    "## 动笔前必须自问",
    "",
...
function buildCreativeOutputFormat(_book: BookConfig, gp: GenreProfile, lengthSpec: LengthSpec): string {
...
function buildOutputFormat(_book: BookConfig, gp: GenreProfile, lengthSpec: LengthSpec): string {
  const resourceRow = gp.numericalSystem
    ? "| 当前资源总量 | X | 与账本一致 |\n| 本章预计增量 | +X（来源） | 无增量写+0 |"
    : "";
