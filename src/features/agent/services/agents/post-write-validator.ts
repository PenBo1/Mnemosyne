...
//
// 迁移要点（import 路径调整）：
// - `../utils/chapter-cadence.js`（analyzeChapterCadence，用于 detectTitleCollapse）
//   → `../utils/chapter-cadence`（chapter-cadence.ts 已迁移到上级 utils 目录）
// - `../models/book-rules.js`（类型 BookRules）→ `@/shared/types/book-rules`
// - `../models/genre-profile.js`（类型 GenreProfile）→ `@/shared/types/genre-profile`
//
// 业务逻辑零改动（十余项确定性校验 + 标题重复坍缩检测 + 段落形状分析 +
// 跨章重复检测 + 叙事人称漂移 + 内部 helper 全部保留）。
//
// 大文件说明：原实现即单文件聚合多项校验，函数间共享 helper（capitalize、
// extractChineseTitleTerms、analyzeParagraphShape 等），拆分会破坏共享。
// 保持原样迁移，后续如需拆分按校验维度（句式/术语/段落/标题/人称）切分。
//
// ES2020 兼容：原文件未使用 .at(-1)，无需调整。
...
import { analyzeChapterCadence } from "../utils/chapter-cadence";
