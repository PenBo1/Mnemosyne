// Observer Prompts（纯函数）。
//
// Observer agent 的 system / user prompt 构造器：
// - buildObserverSystemPrompt: 9 类事实提取规则 + 输出格式
// - buildObserverUserPrompt: 触发提取的单条 user message
//
// 仅类型 import（BookConfig / GenreProfile）。
//
// 迁移要点：
// 1. 仅改 import 路径，去掉 .js 后缀。
// 2. 原实现的 `buildObserverSystemPrompt` 的 `book` 参数声明但从未在函数体内引用
//    （保持签名与其他 builder 一致，留作未来扩展）。Mnemosyne 启用 noUnusedParameters
//    后报错。按 TS 惯例改为 `_book`
//    前缀以标记"故意不用"，函数签名形状不变（caller 仍按位置传参）。
