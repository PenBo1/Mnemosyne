...
//
// 迁移要点（import 路径调整）：
// - `../models/runtime-state.js`（类型）→ `@/shared/types/runtime-state`
...
} from "../utils/hook-stale-detection";
...
export function renderCurrentStateProjection(
  ...
  const slots = [
    { label: layout.labels.location, aliases: ["Current Location", "当前位置"] },
    { label: layout.labels.protagonistState, aliases: ["Protagonist State", "主角状态"] },
    { label: layout.labels.goal, aliases: ["Current Goal", "当前目标"] },
    { label: layout.labels.constraint, aliases: ["Current Constraint", "当前限制"] },
    { label: layout.labels.alliances, aliases: ["Current Alliances", "Current Relationships", "当前敌我"] },
    { label: layout.labels.conflict, aliases: ["Current Conflict", "当前冲突"] },
  ] as const;
