// Input Governance zod schemas —— 纯 zod 定义。
//
// 这些 schema 定义了输入治理的 4 个控制文档结构：
// - ChapterMemo：章节备忘（目标 + 线索引用 + 正文）
// - ChapterIntent：章节意图（目标 + mustKeep/mustAvoid + 风格强调）
// - ContextPackage：上下文包（选中上下文源列表）
// - RuleStack：规则栈（分层规则 + override 边）
// - ChapterTrace：章节追踪（planner/composer 输入 + token 预算）
