    ...
    if let Some(backup) = backup_dir {
        if path.exists() {
            std::fs::create_dir_all(backup)
                .map_err(|e| crate::shared::errors::AppError::internal(format!("Failed to create backup dir: {}", e)))?;
            let backup_path = backup.join(path.file_name().unwrap_or_default());
            std::fs::copy(path, &backup_path)
                .map_err(|e| crate::shared::errors::AppError::internal(format!("Failed to backup: {}", e)))?;
        }
    }
...
pub fn append_to_file(path: &Path, content: &str) -> Result<(), crate::shared::errors::AppError> {
    use std::io::Write;
    let mut file = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open(path)
        .map_err(|e| crate::shared::errors::AppError::internal(format!("Failed to open: {}", e)))?;
    write!(file, "{}", content)
        .map_err(|e| crate::shared::errors::AppError::internal(format!("Failed to write: {}", e)))?;
    Ok(())
}
