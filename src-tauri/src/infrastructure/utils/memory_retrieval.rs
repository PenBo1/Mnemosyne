use crate::infrastructure::utils::story_markdown::*;
...
pub fn retrieve_memory_selection(
    ...
    let active_hooks: Vec<ParsedHook> = all_hooks.iter()
        .filter(|h| !crate::infrastructure::utils::hook_lifecycle::is_terminal_status(&h.status))
        .cloned()
        .collect();
...
pub fn compute_recyclable_hooks(hooks: &[ParsedHook], chapter_number: u32) -> Vec<ParsedHook> {
    hooks.iter()
        .filter(|h| !crate::infrastructure::utils::hook_lifecycle::is_terminal_status(&h.status))
        .filter(|h| !crate::infrastructure::utils::hook_lifecycle::is_future_planned_hook(h.start_chapter, chapter_number))
        .filter(|h| {
            let silence = crate::infrastructure::utils::hook_lifecycle::hook_silence(
                h.start_chapter, h.last_advanced_chapter, chapter_number,
            );
            let threshold = crate::infrastructure::utils::hook_lifecycle::recycle_threshold(
