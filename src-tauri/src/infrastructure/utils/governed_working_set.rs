...
use crate::infrastructure::utils::story_markdown::{ParsedHook, parse_pending_hooks_markdown, render_hook_snapshot};
...
pub fn build_governed_hook_working_set(
    ...
    let working_set: Vec<&ParsedHook> = hooks.iter().filter(|h| {
        selected_ids.contains(&h.hook_id) || agenda_ids.contains(&h.hook_id)
            || crate::infrastructure::utils::hook_lifecycle::is_hook_within_chapter_window(h.last_advanced_chapter, h.start_chapter, chapter_number, 5)
    }).collect();
