...
/// Chat / agent 消息。
///
/// `thinking_content`/`model`/`provider`/`input_tokens`/`output_tokens`/`latency_ms`
/// 仅对 assistant 角色有意义（用于 ChatPage 渲染 thinking 动画与 token 观测），
/// 其他角色这些字段为 `None`/`0`。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Message {
    pub id: String,
...
/// AI 回复消息的元数据写入参数。
///
/// 由 ChatPage 在拿到 LLM 响应后调用 `create_message_with_meta` 时传入。
#[derive(Debug, Clone)]
...
fn map_message(row: &sqlx::sqlite::SqliteRow) -> Message {
    Message {
        ...
        model: row.get(8),
        provider: row.get(9),
        input_tokens: row.get::<i64, usize>(10) as u32,
        output_tokens: row.get::<i64, usize>(11) as u32,
        latency_ms: row.get::<Option<i64>, usize>(12).map(|v| v as u64),
...
impl Database {
    ...
    /// 写入一条无元数据的消息（user/system/tool 角色或不需要元数据的 assistant 文本）。
    ///
    /// AI 回复若需要记录 thinking/model/token，请改用 `create_message_with_meta`。
    ...
    /// 写入带元数据的消息（用于 assistant AI 回复）。
    ///
    /// `meta = None` 时退化为普通消息写入。
    pub async fn create_message_with_meta(
        ...
        .bind(tool_calls).bind(tool_results)
        .bind(thinking).bind(model).bind(provider)
