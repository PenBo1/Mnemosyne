...
use crate::infrastructure::db::types::{CreatePromptRequest, UpdatePromptRequest};
...
pub async fn create_prompt(
...
) -> Result<IpcResponse<crate::infrastructure::db::types::Prompt>, AppError> {
    ...
    let prompt = state.db.create_prompt(req)?;
...
) -> Result<IpcResponse<Vec<crate::infrastructure::db::types::Prompt>>, AppError> {
    tracing::debug!(category = ?category, "list_prompts");
    let prompts = state.db.list_prompts(category.as_deref())?;
...
pub async fn update_prompt(
    ...
    let prompt = state.db.update_prompt(req)?;
...
pub async fn delete_prompt(
    ...
    let deleted = state.db.delete_prompt(&id)?;
