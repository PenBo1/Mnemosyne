...
/// 近似 token 计数（1 token ≈ 4 字符 for English，1.5 字符 for CJK）。
...
impl ContextBudget {
    /// 为给定 context window 大小创建预算。
    /// 分配：10% system、60% history、5% tools、20% response、5% reserved。
    ...
    /// 扣除 system prompt 后可用于 history 的 token。
