...
use crate::core::state::MainAgentSessionState;
use crate::infrastructure::file_storage::fs_utils::validate_id_component;
use crate::AppState;
use tauri::State;
use tauri::Emitter;
use crate::core::agent::main_agent::{AgentStatus, ConfirmationRequest, ConfirmationResponse};
...
pub async fn main_agent_execute(
    ...
    // 用真实的 progress_rx 存储 session 状态
    {
    ...
    // 将 progress_rx 取出，供 spawn 的监听任务使用
    let progress_rx = {
        let mut states = state.main_agent_states.lock().await;
    ...
    let mut tools = crate::core::agent::ToolRegistry::new();
    ...
    let ctx = crate::core::agent::AgentContext {
        ...
        tools: Arc::new(tools),
        memory,
        iteration_budget: Arc::new(crate::core::agent::IterationBudget::new(50)),
        tool_guardrails: Arc::new(tokio::sync::Mutex::new(
            crate::core::agent::ToolCallGuardrailController::new(
                crate::core::agent::ToolGuardrailConfig::default()
            )
        )),
        context_compressor: Arc::new(tokio::sync::Mutex::new(
            crate::core::agent::ContextCompressor::new(
                crate::core::agent::CompressorConfig::default()
            )
        )),
        skill_manager: None,
        user_profile: None,
    ...
    let agent = crate::core::agent::main_agent::AgentLoop::new(
        ctx,
        progress_tx,
        confirmation_req_tx,
