...
impl SkillManager {
    ...
    pub fn add_dir(&mut self, dir: PathBuf) { if !self.dirs.contains(&dir) { self.dirs.push(dir); } }

    pub fn discover(&mut self) -> Result<(), AppError> {
        self.skills.clear();
        let dirs: Vec<PathBuf> = self.dirs.clone();
        for dir in &dirs { if dir.exists() { self.discover_in_dir(dir, 0)?; } }
        Ok(())
    }
    ...
    fn discover_in_dir(&mut self, dir: &Path, depth: usize) -> Result<(), AppError> {
        ...
        for entry in entries {
            ...
            if path.is_dir() { self.discover_in_dir(&path, depth + 1)?; }
            else if name == "SKILL.md" || name == "skill.md" { if let Ok(skill) = self.parse_skill_file(&path) { self.skills.insert(skill.meta.name.clone(), skill); } }
        }
        Ok(())
    }
    ...
    pub fn create_skill(&mut self, meta: SkillMeta, content: &str) -> Result<Skill, AppError> {
        let skill_dir = self.dirs.first().ok_or_else(|| AppError::internal("No skill directory configured"))?;
        ...
        let skill = Skill { meta, content: content.to_string(), path: skill_path.to_string_lossy().to_string(), history: Vec::new() };
    ...
    pub fn update_skill(&mut self, name: &str, meta: SkillMeta, content: &str) -> Result<Skill, AppError> {
        let skill_path = { let existing = self.skills.get(name).ok_or_else(|| AppError::skill_not_found(name))?; existing.path.clone() };
    ...
    pub fn build_index(&self) -> String {
        ...
        for (cat, skills) in cats {
            ...
            for skill in skills { lines.push(format!("- **{}**: {}", skill.meta.name, skill.meta.description)); }
