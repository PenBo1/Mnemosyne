...
fn parse_config(json: &Option<String>) -> Result<LoopConfigDto, AppError> {
    match json {
...
fn parse_state_payload(json: &Option<String>) -> serde_json::Value {
    match json {
        Some(s) if !s.is_empty() => {
...
fn parse_run_result(json: &Option<String>) -> Option<LoopRunResultDto> {
...
fn state_row_to_dto(row: LoopStateRow) -> Result<LoopStateDto, AppError> {
    Ok(LoopStateDto {
        id: row.id,
...
fn pattern_row_to_dto(row: LoopPatternRow) -> Result<LoopPatternDto, AppError> {
...
fn run_row_to_log_dto(row: LoopRunRow) -> LoopRunLogDto {
