...
pub fn assert_safe_book_id(id: &str, context: &str) -> Result<String, crate::errors::AppError> {
    if is_safe_book_id(id) {
        Ok(id.to_string())
    } else {
        Err(crate::errors::AppError::bad_request(format!("{}: invalid book id '{}'", context, id)))
    }
}
