...
pub fn safe_child_path(base: &Path, child: &str) -> Result<PathBuf, crate::errors::AppError> {
    let base_canonical = base.canonicalize()
        .map_err(|e| crate::errors::AppError::internal(format!("Failed to resolve base path: {}", e)))?;
    ...
    if !child_canonical.starts_with(&base_canonical) {
        return Err(crate::errors::AppError::forbidden("Path traversal not allowed"));
    }
...
pub fn assert_safe_truth_file_name(name: &str) -> Result<String, crate::errors::AppError> {
    let trimmed = name.trim();
    if trimmed.is_empty() || trimmed.contains('/') || trimmed.contains('\\') || trimmed.contains("..") || trimmed.contains('\0') {
        return Err(crate::errors::AppError::bad_request(format!("Invalid truth file name: {}", name)));
    }
