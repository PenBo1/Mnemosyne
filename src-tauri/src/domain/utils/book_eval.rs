...
pub fn evaluate_book_quality(
    ...
    let total_critical: usize = audit_results.iter()
        .map(|a| a.issues.iter().filter(|i| i.severity == crate::domain::story::AuditSeverity::Critical).count())
        .sum();
    let total_warning: usize = audit_results.iter()
        .map(|a| a.issues.iter().filter(|i| i.severity == crate::domain::story::AuditSeverity::Warning).count())
        .sum();
