...
impl HarnessConfig {
    pub fn load_from_file(path: &Path) -> Result<Self, crate::errors::AppError> {
        let content = std::fs::read_to_string(path).map_err(|e| {
            crate::errors::AppError::internal(format!("Failed to read harness config: {}", e))
        })?;
        Self::load_from_str(&content)
    }

    pub fn load_from_str(content: &str) -> Result<Self, crate::errors::AppError> {
        serde_json::from_str(content).map_err(|e| {
            crate::errors::AppError::internal(format!("Failed to parse harness config: {}", e))
        })
    }
}
