...
use crate::infra::feedback::{FeedbackStore, ErrorEvent, Severity};
use crate::infra::rag::VectorStore;
use crate::infra::gc::SnapshotGc;
...
impl Scheduler {
    ...
    async fn index_chapter_to_rag(&self, book_id: &str, chapter: u32, content: &str) {
        ...
        rag.index_document(content, &source, &crate::infra::rag::ChunkConfig::default());
    ...
    async fn record_audit_feedback(&self, book_id: &str, chapter: u32, write_result: &crate::domain::story::WriteResult) {
        ...
        for issue in &write_result.audit.issues {
            let severity = match issue.severity {
                crate::domain::story::AuditSeverity::Critical => Severity::Critical,
                crate::domain::story::AuditSeverity::Warning => Severity::Warning,
                crate::domain::story::AuditSeverity::Info => Severity::Warning,
            };
    ...
    pub async fn search_rag(&self, query: &str, top_k: usize) -> Vec<crate::infra::rag::SearchResult> {
        let rag = self.rag_store.lock().await;
        rag.search_hybrid(query, top_k)
    }

    /// Search memory for relevant facts.
    pub async fn search_memory(&self, book_id: &str, query: &str, top_k: usize) -> Vec<crate::domain::agents::base::MemoryEntry> {
