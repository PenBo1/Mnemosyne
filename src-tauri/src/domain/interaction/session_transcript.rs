...
pub fn append_transcript_events(
    ...
    std::fs::create_dir_all(&dir).map_err(|e| crate::errors::AppError::internal(format!("Failed to create session dir: {}", e)))?;
    ...
    let mut file = std::fs::OpenOptions::new()
        ...
        .append(true)
        .open(&path)
        .map_err(|e| crate::errors::AppError::internal(format!("Failed to open transcript: {}", e)))?;

    use std::io::Write;
    for event in events {
        let line = serde_json::to_string(event)
            .map_err(|e| crate::errors::AppError::internal(format!("Failed to serialize event: {}", e)))?;
        writeln!(file, "{}", line)
            .map_err(|e| crate::errors::AppError::internal(format!("Failed to write event: {}", e)))?;
    }
