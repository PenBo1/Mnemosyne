// 每日摘要任务 IPC 命令 —— 暴露 DailySummaryTask 的控制能力给前端。
//
...
//
// 设计参考:pipeline_scheduler_* 命令的 State 模式
// 鉴权:配置参数有边界校验,防止恶意设置过小间隔导致 CPU 空转
...
/// 配置参数边界(防止恶意设置)
const MIN_INTERVAL_MS: u64 = 60_000; // 最低 1 分钟
const MAX_INTERVAL_MS: u64 = 7 * 24 * 60 * 60 * 1000; // 最高 7 天
...
/// 更新配置(运行时生效,无需重启)
///
/// 注意:更新配置不会自动启动任务,需额外调用 `daily_summary_start`
#[tauri::command]
pub async fn daily_summary_update_config(
    ...
    tracing::info!(?config, "Daily summary config updated");
...
pub async fn daily_summary_start(
    ...
    state
        ...
        .await?;
    tracing::info!("Daily summary task: 已启动");
...
pub async fn daily_summary_stop(
    ...
    tracing::info!("Daily summary task: 已停止");
    Ok(IpcResponse::ok(true))
}
